...
`)[2]||"").match(/at\s+(?:.*\s+)?\(?([^:]+):(\d+):(\d+)\)?/),m=l?`${l[1].split("/").pop()}:${l[2]}`:"unknown",u={...s,location:m};return this.warn(e,`[HAPPY-PATH] ${t}`,u,n),o}},p=new Y;var xt={};function Dt(){return typeof __dirname<"u"?__dirname:(0,b.dirname)((0,me.fileURLToPath)(xt.url))}var Mt=Dt(),A=I.get("CLAUDE_MEM_DATA_DIR"),y=process.env.CLAUDE_CONFIG_DIR||(0,b.join)((0,pe.homedir)(),".claude"),os=(0,b.join)(y,"plugins","marketplaces","thedotmack"),is=(0,b.join)(A,"archives"),as=(0,b.join)(A,"logs"),ds=(0,b.join)(A,"trash"),cs=(0,b.join)(A,"backups"),ps=(0,b.join)(A,"modes"),ls=(0,b.join)(A,"settings.json"),ue=(0,b.join)(A,"claude-mem.db"),ms=(0,b.join)(A,"vector-db"),us=(0,b.join)(A,"observer-sessions"),_s=(0,b.join)(y,"settings.json"),Es=(0,b.join)(y,"commands"),gs=(0,b.join)(y,"CLAUDE.md");function _e(r){(0,le.mkdirSync)(r,{recursive:!0})}function Ee(){return(0,b.join)(Mt,"..")}var ge=require("crypto");var Ut=3e4;function Te(r,e,t,s){return(0,ge.createHash)("sha256").update((r||"")+(s||"")+(e||"")+(t||"")).digest("hex").slice(0,16)}function fe(r,e,t){let s=t-Ut;return r.prepare("SELECT id, created_at_epoch FROM observations WHERE content_hash = ? AND created_at_epoch > ?").get(e,s)}var $=class{db;constructor(e=ue){e!==":memory:"&&_e(A),this.db=new he.Database(e),this.db.run("PRAGMA journal_mode = WAL"),this.db.run("PRAGMA synchronous = NORMAL"),this.db.run("PRAGMA foreign_keys = ON"),this.initializeSchema(),this.ensureWorkerPortColumn(),this.ensurePromptTrackingColumns(),this.removeSessionSummariesUniqueConstraint(),this.addObservationHierarchicalFields(),this.makeObservationsTextNullable(),this.createUserPromptsTable(),this.ensureDiscoveryTokensColumn(),this.createPendingMessagesTable(),this.renameSessionIdColumns(),this.repairSessionIdColumnRename(),this.addFailedAtEpochColumn(),this.addOnUpdateCascadeToForeignKeys(),this.addObservationContentHashColumn(),this.addSessionCustomTitleColumn(),this.addObservationBranchColumns(),this.addPendingMessagesBranchColumns()}initializeSchema(){this.db.run(`
    ...
    `),this.db.run("CREATE INDEX IF NOT EXISTS idx_pending_messages_session ON pending_messages(session_db_id)"),this.db.run("CREATE INDEX IF NOT EXISTS idx_pending_messages_status ON pending_messages(status)"),this.db.run("CREATE INDEX IF NOT EXISTS idx_pending_messages_claude_session ON pending_messages(content_session_id)"),this.db.prepare("INSERT OR IGNORE INTO schema_versions (version, applied_at) VALUES (?, ?)").run(16,new Date().toISOString()),p.debug("DB","pending_messages table created successfully")}renameSessionIdColumns(){if(this.db.prepare("SELECT version FROM schema_versions WHERE version = ?").get(17))return;p.debug("DB","Checking session ID columns for semantic clarity rename");let t=0,s=(n,o,i)=>{let a=this.db.query(`PRAGMA table_info(${n})`).all(),d=a.some(m=>m.name===o);return a.some(m=>m.name===i)?!1:d?(this.db.run(`ALTER TABLE ${n} RENAME COLUMN ${o} TO ${i}`),p.debug("DB",`Renamed ${n}.${o} to ${i}`),!0):(p.warn("DB",`Column ${o} not found in ${n}, skipping rename`),!1)};s("sdk_sessions","claude_session_id","content_session_id")&&t++,s("sdk_sessions","sdk_session_id","memory_session_id")&&t++,s("pending_messages","claude_session_id","content_session_id")&&t++,s("observations","sdk_session_id","memory_session_id")&&t++,s("session_summaries","sdk_session_id","memory_session_id")&&t++,s("user_prompts","claude_session_id","content_session_id")&&t++,this.db.prepare("INSERT OR IGNORE INTO schema_versions (version, applied_at) VALUES (?, ?)").run(17,new Date().toISOString()),t>0?p.debug("DB",`Successfully renamed ${t} session ID columns`):p.debug("DB","No session ID column renames needed (already up to date)")}repairSessionIdColumnRename(){this.db.prepare("SELECT version FROM schema_versions WHERE version = ?").get(19)||this.db.prepare("INSERT OR IGNORE INTO schema_versions (version, applied_at) VALUES (?, ?)").run(19,new Date().toISOString())}addFailedAtEpochColumn(){if(this.db.prepare("SELECT version FROM schema_versions WHERE version = ?").get(20))return;this.db.query("PRAGMA table_info(pending_messages)").all().some(n=>n.name==="failed_at_epoch")||(this.db.run("ALTER TABLE pending_messages ADD COLUMN failed_at_epoch INTEGER"),p.debug("DB","Added failed_at_epoch column to pending_messages table")),this.db.prepare("INSERT OR IGNORE INTO schema_versions (version, applied_at) VALUES (?, ?)").run(20,new Date().toISOString())}addOnUpdateCascadeToForeignKeys(){if(!this.db.prepare("SELECT version FROM schema_versions WHERE version = ?").get(21)){p.debug("DB","Adding ON UPDATE CASCADE to FK constraints on observations and session_summaries"),this.db.run("PRAGMA foreign_keys = OFF"),this.db.run("BEGIN TRANSACTION");try{this.db.run("DROP TRIGGER IF EXISTS observations_ai"),this.db.run("DROP TRIGGER IF EXISTS observations_ad"),this.db.run("DROP TRIGGER IF EXISTS observations_au"),this.db.run("DROP TABLE IF EXISTS observations_new"),this.db.run(`
      ...
      `),this.db.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='session_summaries_fts'").all().length>0&&this.db.run(`
          ...
          CREATE TRIGGER IF NOT EXISTS session_summaries_au AFTER UPDATE ON session_summaries BEGIN
            ...
            VALUES (new.id, new.request, new.investigated, new.learned, new.completed, new.next_steps, new.notes);
          END;
        `),this.db.prepare("INSERT OR IGNORE INTO schema_versions (version, applied_at) VALUES (?, ?)").run(21,new Date().toISOString()),this.db.run("COMMIT"),this.db.run("PRAGMA foreign_keys = ON"),p.debug("DB","Successfully added ON UPDATE CASCADE to FK constraints")}catch(t){throw this.db.run("ROLLBACK"),this.db.run("PRAGMA foreign_keys = ON"),t}}}addObservationContentHashColumn(){if(this.db.prepare("SELECT version FROM schema_versions WHERE version = ?").get(22))return;this.db.query("PRAGMA table_info(observations)").all().some(n=>n.name==="content_hash")||(this.db.run("ALTER TABLE observations ADD COLUMN content_hash TEXT"),this.db.run("UPDATE observations SET content_hash = substr(hex(randomblob(8)), 1, 16) WHERE content_hash IS NULL"),this.db.run("CREATE INDEX IF NOT EXISTS idx_observations_content_hash ON observations(content_hash, created_at_epoch)"),p.debug("DB","Added content_hash column to observations table with backfill and index")),this.db.prepare("INSERT OR IGNORE INTO schema_versions (version, applied_at) VALUES (?, ?)").run(22,new Date().toISOString())}addSessionCustomTitleColumn(){if(this.db.prepare("SELECT version FROM schema_versions WHERE version = ?").get(23))return;this.db.query("PRAGMA table_info(sdk_sessions)").all().some(n=>n.name==="custom_title")||(this.db.run("ALTER TABLE sdk_sessions ADD COLUMN custom_title TEXT"),p.debug("DB","Added custom_title column to sdk_sessions table")),this.db.prepare("INSERT OR IGNORE INTO schema_versions (version, applied_at) VALUES (?, ?)").run(23,new Date().toISOString())}addObservationBranchColumns(){if(this.db.prepare("SELECT version FROM schema_versions WHERE version = ?").get(24))return;let t=this.db.query("PRAGMA table_info(observations)").all(),s=t.some(o=>o.name==="branch"),n=t.some(o=>o.name==="commit_sha");s||(this.db.run("ALTER TABLE observations ADD COLUMN branch TEXT"),p.debug("DB","Added branch column to observations table")),n||(this.db.run("ALTER TABLE observations ADD COLUMN commit_sha TEXT"),p.debug("DB","Added commit_sha column to observations table")),this.db.prepare("INSERT OR IGNORE INTO schema_versions (version, applied_at) VALUES (?, ?)").run(24,new Date().toISOString())}addPendingMessagesBranchColumns(){if(this.db.prepare("SELECT version FROM schema_versions WHERE version = ?").get(25))return;let t=this.db.query("PRAGMA table_info(pending_messages)").all(),s=t.some(o=>o.name==="branch"),n=t.some(o=>o.name==="commit_sha");s||(this.db.run("ALTER TABLE pending_messages ADD COLUMN branch TEXT"),p.debug("DB","Added branch column to pending_messages table")),n||(this.db.run("ALTER TABLE pending_messages ADD COLUMN commit_sha TEXT"),p.debug("DB","Added commit_sha column to pending_messages table")),this.db.prepare("INSERT OR IGNORE INTO schema_versions (version, applied_at) VALUES (?, ?)").run(25,new Date().toISOString())}updateMemorySessionId(e,t){this.db.prepare(`
      UPDATE sdk_sessions
    ...
    `).run(e,t,s,n.toISOString(),o).lastInsertRowid}getUserPrompt(e,t){return this.db.prepare(`
      ...
      LIMIT 1
    `).get(e,t)?.prompt_text??null}storeObservation(e,t,s,n,o=0,i){let a=i??Date.now(),d=new Date(a).toISOString(),l=Te(e,s.title,s.narrative),m=fe(this.db,l,a);if(m)return{id:m.id,createdAtEpoch:m.created_at_epoch};let T=this.db.prepare(`
      INSERT INTO observations
      (memory_session_id, project, type, title, subtitle, facts, narrative, concepts,
       files_read, files_modified, prompt_number, discovery_tokens, content_hash, created_at, created_at_epoch)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(e,t,s.type,s.title,s.subtitle,JSON.stringify(s.facts),s.narrative,JSON.stringify(s.concepts),JSON.stringify(s.files_read),JSON.stringify(s.files_modified),n||null,o,l,d,a);return{id:Number(T.lastInsertRowid),createdAtEpoch:a}}storeSummary(e,t,s,n,o=0,i){let a=i??Date.now(),d=new Date(a).toISOString(),m=this.db.prepare(`
      INSERT INTO session_summaries
      (memory_session_id, project, request, investigated, learned, completed,
       next_steps, notes, prompt_number, discovery_tokens, created_at, created_at_epoch)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(e,t,s.request,s.investigated,s.learned,s.completed,s.next_steps,s.notes,n||null,o,d,a);return{id:Number(m.lastInsertRowid),createdAtEpoch:a}}storeObservations(e,t,s,n,o,i=0,a,d,l){let m=a??Date.now(),u=new Date(m).toISOString();return this.db.transaction(()=>{let E=[],h=this.db.prepare(`
        INSERT INTO observations
        (memory_session_id, project, type, title, subtitle, facts, narrative, concepts,
         files_read, files_modified, prompt_number, discovery_tokens, created_at, created_at_epoch,
         branch, commit_sha)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `);for(let _ of s){let g=h.run(e,t,_.type,_.title,_.subtitle,JSON.stringify(_.facts),_.narrative,JSON.stringify(_.concepts),JSON.stringify(_.files_read),JSON.stringify(_.files_modified),o||null,i,u,m,d??null,l??null);E.push(Number(g.lastInsertRowid))}let S=null;if(n){let g=this.db.prepare(`
          INSERT INTO session_summaries
          (memory_session_id, project, request, investigated, learned, completed,
           next_steps, notes, prompt_number, discovery_tokens, created_at, created_at_epoch)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        `).run(e,t,n.request,n.investigated,n.learned,n.completed,n.next_steps,n.notes,o||null,i,u,m);S=Number(g.lastInsertRowid)}return{observationIds:E,summaryId:S,createdAtEpoch:m}})()}storeObservationsAndMarkComplete(e,t,s,n,o,i,a,d=0,l){let m=l??Date.now(),u=new Date(m).toISOString();return this.db.transaction(()=>{let E=[],h=this.db.prepare(`
        INSERT INTO observations
        (memory_session_id, project, type, title, subtitle, facts, narrative, concepts,
         files_read, files_modified, prompt_number, discovery_tokens, created_at, created_at_epoch)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `);for(let g of s){let f=h.run(e,t,g.type,g.title,g.subtitle,JSON.stringify(g.facts),g.narrative,JSON.stringify(g.concepts),JSON.stringify(g.files_read),JSON.stringify(g.files_modified),a||null,d,u,m);E.push(Number(f.lastInsertRowid))}let S;if(n){let f=this.db.prepare(`
          INSERT INTO session_summaries
          (memory_session_id, project, request, investigated, learned, completed,
           next_steps, notes, prompt_number, discovery_tokens, created_at, created_at_epoch)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        `).run(e,t,n.request,n.investigated,n.learned,n.completed,n.next_steps,n.notes,a||null,d,u,m);S=Number(f.lastInsertRowid)}return this.db.prepare(`
      ...
      `).run(m,o),{observationIds:E,summaryId:S,createdAtEpoch:m}})()}getSessionSummariesByIds(e,t={}){if(e.length===0)return[];let{orderBy:s="date_desc",limit:n,project:o}=t,i=s==="date_asc"?"ASC":"DESC",a=n?`LIMIT ${n}`:"",d=e.map(()=>"?").join(","),l=[...e],m=o?`WHERE id IN (${d}) AND project = ?`:`WHERE id IN (${d})`;return o&&l.push(o),this.db.prepare(`
    ...
    `).run(e.content_session_id,e.prompt_number,e.prompt_text,e.created_at,e.created_at_epoch).lastInsertRowid}}};var be=L(require("path"),1);function Se(r){if(!r||r.trim()==="")return p.warn("PROJECT_NAME","Empty cwd provided, using fallback",{cwd:r}),"unknown-project";let e=be.default.basename(r);if(e===""){if(process.platform==="win32"){let s=r.match(/^([A-Z]):\\/i);if(s){let o=`drive-${s[1].toUpperCase()}`;return p.info("PROJECT_NAME","Drive root detected",{cwd:r,projectName:o}),o}}return p.warn("PROJECT_NAME","Root directory detected, using fallback",{cwd:r}),"unknown-project"}return e}function Oe(r,e){return r.prepare(`
    SELECT DISTINCT commit_sha
    FROM observations
    WHERE project = ? AND commit_sha IS NOT NULL
  `).all(e).map(n=>n.commit_sha)}var P=require("child_process");var Re=require("child_process");async function Ae(r){try{return(0,Re.execSync)("git rev-parse --is-inside-work-tree",{cwd:r,encoding:"utf8",stdio:["pipe","pipe","pipe"],timeout:5e3}).trim()==="true"}catch{return!1}}async function kt(r){try{return(0,P.execSync)("git rev-parse HEAD",{cwd:r,encoding:"utf8",stdio:["pipe","pipe","pipe"],timeout:5e3}).trim()||null}catch{return null}}var F=100,wt=500;async function $t(r,e,t){if(e.length===0)return[];if(e.length>wt){p.debug("DB",`resolveAncestorCommits: using git-log optimization for ${e.length} candidates`);let n=Ft(r,e,t);if(n!==null)return p.debug("DB",`resolveAncestorCommits: ${n.length}/${e.length} candidates visible`),n;p.debug("DB",`resolveAncestorCommits: git-log failed, falling back to batched merge-base for ${e.length} candidates`)}if(e.length>F){p.debug("DB",`resolveAncestorCommits: batching ${e.length} candidates in groups of ${F}`);let n=[];for(let o=0;o<e.length;o+=F){let i=e.slice(o,o+F),a=await Ne(r,i,t);n.push(...a)}return p.debug("DB",`resolveAncestorCommits: ${n.length}/${e.length} candidates visible`),n}let s=await Ne(r,e,t);return p.debug("DB",`resolveAncestorCommits: ${s.length}/${e.length} candidates visible`),s}async function Ne(r,e,t){return(await Promise.all(e.map(async n=>{try{return(0,P.execSync)(`git merge-base --is-ancestor ${n} ${r}`,{cwd:t,encoding:"utf8",stdio:["pipe","pipe","pipe"],timeout:5e3}),n}catch{return null}}))).filter(n=>n!==null)}function Ft(r,e,t){try{let s=(0,P.execSync)(`git log --format=%H ${r}`,{cwd:t,encoding:"utf8",stdio:["pipe","pipe","pipe"],timeout:3e4,maxBuffer:52428800}).trim(),n=new Set(s.split(`
`));return e.filter(o=>n.has(o))}catch{return p.debug("DB","resolveViaGitLog failed, signaling caller to use batched merge-base"),null}}async function Ce(r,e){if(!await Ae(e))return null;let s=await kt(e);return s===null?null:r.length===0?[]:$t(s,r,e)}var Ie=L(require("path"),1),ye=require("os");var M=require("fs"),X=require("path");var R=class r{static instance=null;activeMode=null;modesDir;constructor(){let e=Ee(),t=[(0,X.join)(e,"modes"),(0,X.join)(e,"..","plugin","modes")],s=t.find(n=>(0,M.existsSync)(n));this.modesDir=s||t[0]}static getInstance(){return r.instance||(r.instance=new r),r.instance}parseInheritance(e){let t=e.split("--");if(t.length===1)return{hasParent:!1,parentId:"",overrideId:""};if(t.length>2)throw new Error(`Invalid mode inheritance: ${e}. Only one level of inheritance supported (parent--override)`);return{hasParent:!0,parentId:t[0],overrideId:e}}isPlainObject(e){return e!==null&&typeof e=="object"&&!Array.isArray(e)}deepMerge(e,t){let s={...e};for(let n in t){let o=t[n],i=e[n];this.isPlainObject(o)&&this.isPlainObject(i)?s[n]=this.deepMerge(i,o):s[n]=o}return s}loadModeFile(e){let t=(0,X.join)(this.modesDir,`${e}.json`);if(!(0,M.existsSync)(t))throw new Error(`Mode file not found: ${t}`);let s=(0,M.readFileSync)(t,"utf-8");return JSON.parse(s)}loadMode(e){let t=this.parseInheritance(e);if(!t.hasParent)try{let d=this.loadModeFile(e);return this.activeMode=d,p.debug("SYSTEM",`Loaded mode: ${d.name} (${e})`,void 0,{types:d.observation_types.map(l=>l.id),concepts:d.observation_concepts.map(l=>l.id)}),d}catch{if(p.warn("SYSTEM",`Mode file not found: ${e}, falling back to 'code'`),e==="code")throw new Error("Critical: code.json mode file missing");return this.loadMode("code")}let{parentId:s,overrideId:n}=t,o;try{o=this.loadMode(s)}catch{p.warn("SYSTEM",`Parent mode '${s}' not found for ${e}, falling back to 'code'`),o=this.loadMode("code")}let i;try{i=this.loadModeFile(n),p.debug("SYSTEM",`Loaded override file: ${n} for parent ${s}`)}catch{return p.warn("SYSTEM",`Override file '${n}' not found, using parent mode '${s}' only`),this.activeMode=o,o}if(!i)return p.warn("SYSTEM",`Invalid override file: ${n}, using parent mode '${s}' only`),this.activeMode=o,o;let a=this.deepMerge(o,i);return this.activeMode=a,p.debug("SYSTEM",`Loaded mode with inheritance: ${a.name} (${e} = ${s} + ${n})`,void 0,{parent:s,override:n,types:a.observation_types.map(d=>d.id),concepts:a.observation_concepts.map(d=>d.id)}),a}getActiveMode(){if(!this.activeMode)throw new Error("No mode loaded. Call loadMode() first.");return this.activeMode}getObservationTypes(){return this.getActiveMode().observation_types}getObservationConcepts(){return this.getActiveMode().observation_concepts}getTypeIcon(e){return this.getObservationTypes().find(s=>s.id===e)?.emoji||"\u{1F4DD}"}getWorkEmoji(e){return this.getObservationTypes().find(s=>s.id===e)?.work_emoji||"\u{1F4DD}"}validateType(e){return this.getObservationTypes().some(t=>t.id===e)}getTypeLabel(e){return this.getObservationTypes().find(s=>s.id===e)?.label||e}};function q(){let r=Ie.default.join((0,ye.homedir)(),".claude-mem","settings.json"),e=I.loadFromFile(r),t=e.CLAUDE_MEM_MODE,s=t==="code"||t.startsWith("code--"),n,o;if(s)n=new Set(e.CLAUDE_MEM_CONTEXT_OBSERVATION_TYPES.split(",").map(i=>i.trim()).filter(Boolean)),o=new Set(e.CLAUDE_MEM_CONTEXT_OBSERVATION_CONCEPTS.split(",").map(i=>i.trim()).filter(Boolean));else{let i=R.getInstance().getActiveMode();n=new Set(i.observation_types.map(a=>a.id)),o=new Set(i.observation_concepts.map(a=>a.id))}return{totalObservationCount:parseInt(e.CLAUDE_MEM_CONTEXT_OBSERVATIONS,10),fullObservationCount:parseInt(e.CLAUDE_MEM_CONTEXT_FULL_COUNT,10),sessionCount:parseInt(e.CLAUDE_MEM_CONTEXT_SESSION_COUNT,10),showReadTokens:e.CLAUDE_MEM_CONTEXT_SHOW_READ_TOKENS==="true",showWorkTokens:e.CLAUDE_MEM_CONTEXT_SHOW_WORK_TOKENS==="true",showSavingsAmount:e.CLAUDE_MEM_CONTEXT_SHOW_SAVINGS_AMOUNT==="true",showSavingsPercent:e.CLAUDE_MEM_CONTEXT_SHOW_SAVINGS_PERCENT==="true",observationTypes:n,observationConcepts:o,fullObservationField:e.CLAUDE_MEM_CONTEXT_FULL_FIELD,showLastSummary:e.CLAUDE_MEM_CONTEXT_SHOW_LAST_SUMMARY==="true",showLastMessage:e.CLAUDE_MEM_CONTEXT_SHOW_LAST_MESSAGE==="true"}}var c={reset:"\x1B[0m",bright:"\x1B[1m",dim:"\x1B[2m",cyan:"\x1B[36m",green:"\x1B[32m",yellow:"\x1B[33m",blue:"\x1B[34m",magenta:"\x1B[35m",gray:"\x1B[90m",red:"\x1B[31m"},ve=4,K=1;function J(r){let e=(r.title?.length||0)+(r.subtitle?.length||0)+(r.narrative?.length||0)+JSON.stringify(r.facts||[]).length;return Math.ceil(e/ve)}function Q(r){let e=r.length,t=r.reduce((i,a)=>i+J(a),0),s=r.reduce((i,a)=>i+(a.discovery_tokens||0),0),n=s-t,o=s>0?Math.round(n/s*100):0;return{totalObservations:e,totalReadTokens:t,totalDiscoveryTokens:s,savings:n,savingsPercent:o}}function Pt(r){return R.getInstance().getWorkEmoji(r)}function v(r,e){let t=J(r),s=r.discovery_tokens||0,n=Pt(r.type),o=s>0?`${n} ${s.toLocaleString()}`:"-";return{readTokens:t,discoveryTokens:s,discoveryDisplay:o,workEmoji:n}}function j(r){return r.showReadTokens||r.showWorkTokens||r.showSavingsAmount||r.showSavingsPercent}var Le=L(require("path"),1),B=require("fs");function De(r){return r==null?{clause:"",params:[]}:r.length===0?{clause:"AND commit_sha IS NULL",params:[]}:{clause:`AND (commit_sha IS NULL OR commit_sha IN (${r.map(()=>"?").join(",")}))`,params:[...r]}}function z(r,e,t,s){let n=Array.from(t.observationTypes),o=n.map(()=>"?").join(","),i=Array.from(t.observationConcepts),a=i.map(()=>"?").join(","),d=De(s);return r.db.prepare(`
    SELECT
  ...
  `).all(e,...n,...i,...d.params,t.totalObservationCount)}function Z(r,e,t){return r.db.prepare(`
    ...
    ORDER BY created_at_epoch DESC
    LIMIT ?
  `).all(e,t.sessionCount+K)}function Me(r,e,t,s){let n=Array.from(t.observationTypes),o=n.map(()=>"?").join(","),i=Array.from(t.observationConcepts),a=i.map(()=>"?").join(","),d=De(s),l=e.map(()=>"?").join(",");return r.db.prepare(`
    SELECT
      id, memory_session_id, type, title, subtitle, narrative,
      facts, concepts, files_read, files_modified, discovery_tokens,
...
`}function pt(r,e,t,s){let n=[];return s?n.push(...Qe(r)):n.push(...$e(r)),s?n.push(...ze()):n.push(...Fe()),s?n.push(...Ze()):n.push(...Pe()),s?n.push(...et()):n.push(...Xe()),j(t)&&(s?n.push(...tt(e,t)):n.push(...je(e,t))),n}var se=L(require("path"),1);function W(r){if(!r)return[];try{let e=JSON.parse(r);return Array.isArray(e)?e:[]}catch(e){return p.debug("PARSER","Failed to parse JSON array, using empty fallback",{preview:r?.substring(0,50)},e),[]}}function mt(r){return new Date(r).toLocaleString("en-US",{month:"short",day:"numeric",hour:"numeric",minute:"2-digit",hour12:!0})}function ut(r){return new Date(r).toLocaleString("en-US",{hour:"numeric",minute:"2-digit",hour12:!0})}function _t(r){return new Date(r).toLocaleString("en-US",{month:"short",day:"numeric",year:"numeric"})}function lt(r,e){return se.default.isAbsolute(r)?se.default.relative(e,r):r}function Et(r,e,t){let s=W(r);if(s.length>0)return lt(s[0],e);if(t){let n=W(t);if(n.length>0)return lt(n[0],e)}return"General"}function Bt(r){let e=new Map;for(let s of r){let n=s.type==="observation"?s.data.created_at:s.data.displayTime,o=_t(n);e.has(o)||e.set(o,[]),e.get(o).push(s)}let t=Array.from(e.entries()).sort((s,n)=>{let o=new Date(s[0]).getTime(),i=new Date(n[0]).getTime();return o-i});return new Map(t)}function Gt(r,e){return e.fullObservationField==="narrative"?r.narrative:r.facts?W(r.facts).join(`
`):null}function Ht(r,e,t,s,n,o){let i=[];o?i.push(...st(r)):i.push(...Be(r));let a=null,d="",l=!1;for(let m of e)if(m.type==="summary"){l&&(i.push(""),l=!1,a=null,d="");let u=m.data,T=mt(u.displayTime);o?i.push(...it(u,T)):i.push(...Ve(u,T))}else{let u=m.data,T=Et(u.files_modified,n,u.files_read),E=ut(u.created_at),h=E!==d,S=h?E:"";d=E;let _=t.has(u.id);if(T!==a&&(l&&i.push(""),o?i.push(...rt(T)):i.push(...Ge(T)),a=T,l=!0),_){let g=Gt(u,s);o?i.push(...ot(u,E,h,g,s)):(l&&!o&&(i.push(""),l=!1),i.push(...We(u,S,g,s)),a=null)}else o?i.push(nt(u,E,h,s)):i.push(He(u,S,s))}return l&&i.push(""),i}function gt(r,e,t,s,n){let o=[],i=Bt(r);for(let[a,d]of i)o.push(...Ht(a,d,e,t,s,n));return o}function Tt(r,e,t){return!(!r.showLastSummary||!e||!!!(e.investigated||e.learned||e.completed||e.next_steps)||t&&e.created_at_epoch<=t.created_at_epoch)}function ft(r,e){let t=[];return e?(t.push(...U("Investigated",r.investigated,c.blue)),t.push(...U("Learned",r.learned,c.yellow)),t.push(...U("Completed",r.completed,c.green)),t.push(...U("Next Steps",r.next_steps,c.magenta))):(t.push(...x("Investigated",r.investigated)),t.push(...x("Learned",r.learned)),t.push(...x("Completed",r.completed)),t.push(...x("Next Steps",r.next_steps))),t}function ht(r,e){return e?at(r):Ye(r)}function bt(r,e,t){return!j(e)||r.totalDiscoveryTokens<=0||r.savings<=0?[]:t?dt(r.totalDiscoveryTokens,r.totalReadTokens):qe(r.totalDiscoveryTokens,r.totalReadTokens)}var Wt=St.default.join((0,Ot.homedir)(),".claude","plugins","marketplaces","thedotmack","plugin",".install-version");function Vt(){try{return new $}catch(r){if(r.code==="ERR_DLOPEN_FAILED"){try{(0,Rt.unlinkSync)(Wt)}catch(e){p.debug("SYSTEM","Marker file cleanup failed (may not exist)",{},e)}return p.error("SYSTEM","Native module rebuild needed - restart Claude Code to auto-fix"),null}throw r}}function Yt(r,e){return e?ct(r):Ke(r)}function qt(r,e,t,s,n,o,i){let a=[],d=Q(e);a.push(...pt(r,d,s,i));let l=t.slice(0,s.sessionCount),m=Ue(l,t),u=te(e,m),T=ke(e,s.fullObservationCount);a.push(...gt(u,T,s,n,i));let E=t[0],h=e[0];Tt(s,E,h)&&a.push(...ft(E,i));let S=ee(e,s,o,n);return a.push(...ht(S,i)),a.push(...bt(d,s,i)),a.join(`
`).trimEnd()}async function re(r,e=!1){let t=q(),s=r?.cwd??process.cwd(),n=Se(s),o=r?.projects||[n],i=Vt();if(!i)return"";try{let a=null;try{let m=new Set;for(let u of o){let T=Oe(i.db,u);for(let E of T)m.add(E)}a=await Ce(Array.from(m),s)}catch(m){p.debug("CONTEXT","Branch visibility resolution failed, showing all observations",{},m),a=null}let d=o.length>1?Me(i,o,t,a):z(i,n,t,a),l=o.length>1?xe(i,o,t):Z(i,n,t);return d.length===0&&l.length===0?Yt(n,e):qt(n,d,l,t,s,r?.session_id,e)}finally{i.close()}}0&&(module.exports={generateContext});
