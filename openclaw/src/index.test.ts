  ...
  it("connects to SSE stream and sends compact new_observation feed messages", async () => {
    ...
    const observation = {
      ...
      observation: {
        ...
        project: "test",
        prompt_number: 1,
        narrative: "This is the fuller context that should be visible in feed messages.",
        facts: JSON.stringify(["Fact one is preserved", "Fact two is preserved"]),
        concepts: JSON.stringify(["rich-feed"]),
    ...
    assert.ok(!sentMessages[0].text.includes("This is the fuller context"));
    assert.ok(!sentMessages[0].text.includes("Facts"));
    assert.ok(!sentMessages[0].text.includes("Fact one is preserved"));
    assert.ok(!sentMessages[0].text.includes("Concepts: rich-feed"));
